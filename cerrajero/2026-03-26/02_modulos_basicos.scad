...
module placa_cuadrada_geom(lado, esp, taladros=false){
    difference(){
        ...
        if (taladros)
            for (sx=[-1,1], sy=[-1,1]){
                translate([
                    sx*(lado/2 - taladro_margen),
                    sy*(lado/2 - taladro_margen),
                    -1
                ])
                    cylinder(h=esp+2, d=taladro_d, $fn=40);
            }
    }
}

module placa_base_geom(){
    placa_cuadrada_geom(base_lado, base_esp, true);
}
...
module pletina_cabeza_geom(){
    placa_cuadrada_geom(pletina_cabeza_lado, pletina_cabeza_esp, false);
}

module pletina_cabeza(x,y,z0){
    translate([x,y,z0]) pletina_cabeza_geom();
}
...
module pilarC(x,y,h){
    ...
    if (ver_pletinas_cabeza)
        pletina_cabeza(x, y, base_esp + h);
}
...
module pilar_sin_baseC(x,y,z0,h){
    ...
    if (ver_pletinas_cabeza)
        pletina_cabeza(x, y, z0 + h);
}

module apoyo_corto_con_pletinas(x,y,z0,h){
    pletina_cabeza(x, y, z0);

    translate([x - pilar_ext/2, y - pilar_ext/2, z0 + pletina_cabeza_esp])
        tubo(pilar_ext, pilar_ext, h, pilar_esp);

    if (ver_pletinas_cabeza)
        pletina_cabeza(x, y, z0 + pletina_cabeza_esp + h);
}

module viga_segmento_tramo(p0, p1, z_inf, d_ini=0, d_fin=-1){
    largo_total = segmento_largo(p0, p1);
    dist_fin = d_fin >= 0 ? d_fin : largo_total;

    if (dist_fin > d_ini)
        viga_segmento(
            punto_en_segmento(p0, p1, d_ini),
            punto_en_segmento(p0, p1, dist_fin),
            z_inf
        );
}
...
module secXc(x_ini, x_fin, y_centro, z_inf){
    largo = x_fin - x_ini;
    if (largo > 0)
        translate([x_ini, y_centro - sec_b/2, z_inf])
            tubo(largo, sec_b, sec_h, sec_esp);
}

module secYc(x_centro, y_ini, y_fin, z_inf){
    largo = y_fin - y_ini;
    if (largo > 0)
        translate([x_centro - sec_b/2, y_ini, z_inf])
            tubo(sec_b, largo, sec_h, sec_esp);
}
