...
module cartela_viga_pilar_segmento(p0, p1, z_top, ala=260, esp=8) {
    largo = segmento_largo(p0, p1);
    ang = segmento_angulo(p0, p1);

    if (largo > 0)
        translate([p0[0], p0[1], z_top - cartela_retr])
            rotate([0, 0, ang])
                translate([0, -esp/2, 0])
                    rotate([90, 0, 0])
...
module capa_cartelas() {
    ...
    p6_p = [p6_x, p6_y];
    p8_p = [p8_x, p8_y];
    apoyo_cama_p = [apoyo_cama_x, apoyo_cama_y];

    color([0.18,0.18,0.20]) {
        // Plataforma inferior: cartelas en los cuatro nudos reales.
        cartela_viga_pilar_segmento(plataforma_grande_sw, plataforma_grande_se, z_viga_grande + viga_h, cartela_ala, cartela_esp);
        cartela_viga_pilar_segmento(plataforma_grande_sw, plataforma_grande_nw, z_viga_grande + viga_h, cartela_ala, cartela_esp);

        cartela_viga_pilar_segmento(plataforma_grande_nw, plataforma_grande_sw, z_viga_grande + viga_h, cartela_ala, cartela_esp);
        cartela_viga_pilar_segmento(plataforma_grande_nw, plataforma_grande_ne, z_viga_grande + viga_h, cartela_ala, cartela_esp);

        cartela_viga_pilar_segmento(plataforma_grande_ne, plataforma_grande_nw, z_viga_grande + viga_h, cartela_ala, cartela_esp);
        cartela_viga_pilar_segmento(plataforma_grande_ne, plataforma_grande_se, z_viga_grande + viga_h, cartela_ala, cartela_esp);

        // Cama/plataforma superior: cartelas en los apoyos reales.
        cartela_viga_pilar_segmento(p8_p, cama_nw, z_viga_pequena + viga_h, 220, cartela_esp);
        cartela_viga_pilar_segmento(p8_p, cama_se, z_viga_pequena + viga_h, 220, cartela_esp);

        cartela_viga_pilar_segmento(p5_p, cama_nw, z_viga_pequena + viga_h, 220, cartela_esp);
        cartela_viga_pilar_segmento(p5_p, p6_p, z_viga_pequena + viga_h, 220, cartela_esp);

        cartela_viga_pilar_segmento(p6_p, p5_p, z_viga_pequena + viga_h, 220, cartela_esp);

        cartela_viga_pilar_segmento(apoyo_cama_p, cama_sw, z_viga_pequena + viga_h, 220, cartela_esp);
        cartela_viga_pilar_segmento(apoyo_cama_p, p6_p, z_viga_pequena + viga_h, 220, cartela_esp);
    }
}
...
module capa_etiquetas_pilares() {
    ...
    etiqueta_pilar("P3", esc_recta_x_ini,   y1_min,           -70,  90);
...
module capa_cotas() {
    ...
    cota_x(esc_recta_x_ini, esc_recta_x_llegada, esc_recta_y_llegada - 60, zc, str(ancho_escalera, " mm"));
}
